...
//    val actions = Seq(opts.addOpt, opts.listOpt, opts.removeOpt).count(option => opts.options.has(option))
//    if(actions != 1)
//      CommandLineUtils.printUsageAndDie(opts.parser, "Command must include exactly one action: --list, --add, --remove.")
  ...
  private def addAcl(authZ: Authorizer, opts: TopicCommandOptions): Unit = {
  ...
  private def removeAcl(authZ: Authorizer, opts: TopicCommandOptions): Unit = {
    val acls: Set[Acl] = getAcl(opts)
    println("Removing acls: " + nl + acls.map("\t" + _).mkString(nl) + nl)
    authZ.removeAcls(acls, getResource(opts))
    listAcl(authZ, opts)
  }

  private def listAcl(authZ: Authorizer, opts: TopicCommandOptions): Unit = {
  ...
  private def getAcl(opts: TopicCommandOptions) : Set[Acl] = {
    println(opts.options.valueOf(opts.allowPrincipalsOpt))
    println(opts.options.valueOf(opts.allowHostsOpt))
    println(opts.options.valueOf(opts.operationsOpt))
    println(opts.options.valueOf(opts.allowHostsOpt).toString.split(delimter))
    println(opts.options.valueOf(opts.operationsOpt).toString.split(delimter))
    println(opts.options.valueOf(opts.allowPrincipalsOpt).toString.split(delimter))
  ...
  private def getResource(opts: TopicCommandOptions) : String = {
  ...
  class TopicCommandOptions(args: Array[String]) {
    ...
    val allTopicLevelOpts: Set[OptionSpec[_]] = Set(addOpt, removeOpt, listOpt)
