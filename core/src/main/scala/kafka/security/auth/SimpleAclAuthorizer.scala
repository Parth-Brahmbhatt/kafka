...
class SimpleAclAuthorizer extends Authorizer with Logging {
  ...
  override def initialize(kafkaConfig: KafkaConfig): Unit = {
    ...
    zkClient = ZkUtils.createZkClient(zkUrl, kafkaConfig.zkConnectionTimeoutMs, kafkaConfig.zkConnectionTimeoutMs)
  ...
  override def authorize(session: Session, operation: Operation, resource: Resource): Boolean = {
    ...
    if(aclMatch(session, Set(operation), resource, principal, remoteAddress, Deny, acls))
      return false


    //now check if there is any allow acl that will allow this operation.
    if(aclMatch(session, ops, resource, principal, remoteAddress, Allow, acls))
      return true
  ...
  private def aclMatch(session: Session, operation: Set[Operation], resource: Resource, principal: KafkaPrincipal, remoteAddress: String, permissionType: PermissionType, acls: Set[Acl]): Boolean = {
    for (acl: Acl <- acls) {
      if (acl.permissionType.equals(permissionType)
        ...
        && (acl.operations.intersect(operation).nonEmpty  || acl.operations.contains(All))
