...
object PermissionType {
  def fromString(permissionType: String) : PermissionType = {
    return permissionType match {
      case permissionType if permissionType.equalsIgnoreCase(Allow.name) => Allow
      case permissionType if permissionType.equalsIgnoreCase(Deny.name) => Deny
    }
  }
