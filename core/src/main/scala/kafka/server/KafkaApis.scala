...
import kafka.security.auth.{ResourceType, Resource, Operation, Authorizer}
...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleConsumerMetadataRequest(request: RequestChannel.Request) {
    ...
    val topicResponses = metadataCache.getTopicMetadata(Set(OffsetManager.OffsetsTopicName), request.securityProtocol)
