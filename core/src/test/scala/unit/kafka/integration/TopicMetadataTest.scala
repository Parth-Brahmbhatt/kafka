...
class TopicMetadataTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  override def setUp() {
    ...
    val configs = props.map(new KafkaConfig(_))
    server1 = TestUtils.createServer(configs.head)
    brokerEndPoints = Seq(new Broker(server1.config.brokerId, server1.config.hostName, server1.config.port).getBrokerEndPoint(SecurityProtocol.PLAINTEXT))
  }
