...
import kafka.api._
import kafka.utils.{TestUtils, ZkUtils, Utils}
...
class LeaderElectionTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  override def setUp() {
    ...
    val configProps1 = TestUtils.createBrokerConfig(brokerId1, enableControlledShutdown = false)
    val configProps2 = TestUtils.createBrokerConfig(brokerId2, enableControlledShutdown = false)
  ...
  def testLeaderElectionWithStaleControllerEpoch() {
    ...
    val controllerConfig = new KafkaConfig(TestUtils.createBrokerConfig(controllerId))
