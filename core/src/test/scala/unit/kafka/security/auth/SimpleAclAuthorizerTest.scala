...
import kafka.security.auth.{KafkaPrincipal, _}
...
class SimpleAclAuthorizerTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  def testTopicAcl(): Unit = {
    ...
    val acl1: Acl = new Acl(Set(user1), Allow, Set[String](host1, host2), Set[Operation](Read))

    //user1 does not have  READ access from host1.
    val acl2: Acl = new Acl(Set(user1), Deny, Set[String](host1), Set[Operation](Read))

    //user1 has Write access from host1 only.
    val acl3: Acl = new Acl(Set(user1), Allow, Set[String](host1), Set[Operation](Write))

    //user1 has DESCRIBE access from all hosts.
    val acl4: Acl = new Acl(Set(user1), Allow, Set[String](Acl.WildCardHost), Set[Operation](Describe))

    //user2 has READ access from all hosts.
    val acl5: Acl = new Acl(Set(user2), Allow, Set[String](Acl.WildCardHost), Set[Operation](Read))

    //user3 has WRITE access from all hosts.
    val acl6: Acl = new Acl(Set(user3), Allow, Set[String](Acl.WildCardHost), Set[Operation](Write))
  ...
  def testDenyTakesPrecedence(): Unit = {
    ...
    val denyAcl: Acl = new Acl(Set(user), Deny, Set[String](host), Set[Operation](All))
  ...
  def testSuperUserHasAccess(): Unit = {
    val denyAllAcl: Acl = new Acl(Set(Acl.WildCardPrincipal), Deny, Set[String](Acl.WildCardHost), Set[Operation](All))
  ...
  def testFailOpenOnProgrammingErrors(): Unit = {
    assertTrue("null session should fail open.", simpleAclAuthorizer.authorize(null, Read, resource))
    assertTrue("null principal should fail open.", simpleAclAuthorizer.authorize(new Session(null, testHostName), Read, resource))
    assertTrue("null host should fail open.", simpleAclAuthorizer.authorize(new Session(testPrincipal, null), Read, resource))
  }
  ...
  def testAclManagementAPIs(): Unit = {
    ...
    val acl1: Acl = new Acl(Set(user1, user2), Allow, Set[String](host1, host2), Set[Operation](Read, Write))
    ...
    val acl2: Acl = new Acl(Set(user2), Allow, Set[String](Acl.WildCardHost), Set[Operation](Read))
    ...
    val acl3: Acl = new Acl(Set(user2), Allow, Set[String](Acl.WildCardHost), Set[Operation](Read))
