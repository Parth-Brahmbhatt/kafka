...
class AclCommandTest extends JUnit3Suite with ZooKeeperTestHarness with Logging {
  ...
  def testAclCli() {
    ...
    val operation1 = Read
    val operation2 = Write
    val host1 = "host1"
    val host2 = "host2"
    val acl1 = new Acl(Set[KafkaPrincipal](user1, user2), Allow, Set[String](host1, host2), Set[Operation](operation1, operation2))
    val acl2 = new Acl(Set[KafkaPrincipal](user1, user2), Deny, Set[String](host1, host2), Set[Operation](operation1, operation2))
    ...
    val args: Array[String] = Array("--config", config.getPath ,
      "--topic", topic,
      "--allowprincipals",user1.toString + AclCommand.delimeter + user2.toString,
      "--allowhosts", host1 + AclCommand.delimeter + host2,
      "--denyprincipals", user1.toString + AclCommand.delimeter + user2.toString,
      "--denyhosts", host1 + AclCommand.delimeter + host2,
      "--operations", operation1.name + AclCommand.delimeter + operation2.name)

    AclCommand.main(args :+ "--add")
    assertEquals(acls, getAuthorizer.getAcls(new Resource(Topic, topic)))

    //Fails transiently , needs investigation.
    AclCommand.main(args :+ "--remove")
    assertTrue(getAuthorizer.getAcls(new Resource(Topic, topic)).isEmpty)
  }
